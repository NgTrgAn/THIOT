...
//   USART_Tx( USART0, buffer );
   uint32_t startTime, endTime, transmissionTime;
   char message[100];
   int i = 0;

   startTime = TIMER_CounterGet(TIMER0);
   message[i++] = buffer;
   while (message[i - 1] != '\n') {
         message[i] = USART_Rx(USART0);
         i++;
     }
   endTime = TIMER_CounterGet(TIMER0);
   float transmissionTimeins = (float) (endTime - startTime)/40000000; //trans to seconds with FCLK is 40MHZ
   transmissionTime = endTime - startTime;

       char response[50];
       sprintf(response, "Transmission time: %lu cycles (%.6f seconds)\n", transmissionTime, transmissionTimeins);
