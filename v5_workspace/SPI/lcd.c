...
sl_status_t lcd_tx(const struct sl_memlcd_t *device, const void *data, unsigned int row_start, unsigned int row_count) {
  ...
  // Set SCS (chip select) to start transmission
  GPIO_PinOutSet(SL_MEMLCD_SPI_CS_PORT, SL_MEMLCD_SPI_CS_PIN);

  // SCS setup time
  sl_udelay_wait(device->setup_us);

      // Gửi lệnh và dữ liệu cho nhiều dòng
      for (unsigned int row = 0; row < row_count; row++) {
          // Chuẩn bị lệnh cập nhật cho dòng hiện tại
          cmd = CMD_UPDATE | ((row_start + row + 1) << 8); // Shift row number as per LCD protocol
          sli_memlcd_spi_tx(&spi_handle, &cmd, 2);

          // Gửi dữ liệu của dòng hiện tại
          sli_memlcd_spi_tx(&spi_handle, data_ptr + row * device->width / 8, device->width / 8);

          // Đợi hoàn tất truyền SPI
          sli_memlcd_spi_wait(&spi_handle);
      }
  // SCS hold time sau khi truyền
  sl_udelay_wait(device->hold_us);

  // Clear SCS để kết thúc truyền
  GPIO_PinOutClear(SL_MEMLCD_SPI_CS_PORT, SL_MEMLCD_SPI_CS_PIN);
  return SL_STATUS_OK;
}

sl_status_t lcd_tx_1(const struct sl_memlcd_t *device, const void *data, unsigned int row_start, unsigned int row_count) {
