...
uint32_t ble_broadcast_cycle = 1000; // default 5 seconds
...
void process_uart_command(uint8_t command) {
  switch (command) {
    case 0x01: // Set temperature/humidity measurement cycle
      ...
      break;
    case 0x02: // Set BLE broadcast cycle
      {
        uint8_t cycle_high = USART_Rx(USART0);
        uint8_t cycle_low = USART_Rx(USART0);
        ble_broadcast_cycle = (cycle_high << 8) | cycle_low;
      }
      break;
    case 0x03: // Get temperature and humidity
      {
