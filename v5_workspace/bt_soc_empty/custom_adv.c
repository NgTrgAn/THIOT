...
void update_adv_data(CustomAdv_t *pData, uint8_t advertising_set_handle, uint32_t student_id,uint16_t period)
{
  sl_status_t sc;
  ...
  pData->company_HI = period & 0xFF;
  pData->company_LO = (period >> 8) & 0xFF;
  // Set custom advertising payload 
  sc = sl_bt_legacy_advertiser_set_data(advertising_set_handle, 0, pData->data_size, (const uint8_t *)pData);
  app_assert(sc == SL_STATUS_OK,
                  "[E: 0x%04x] Failed to set advertising data\n",
